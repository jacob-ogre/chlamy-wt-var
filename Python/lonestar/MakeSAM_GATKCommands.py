...
def main():
    ...
    COMMENTS:
        Globs for .sam files using the patterns *_fq.sam.  Throughout, this 
    ...
    with open(outfil, "wb") as out:
        counter = 0
        for fil in glob.glob(srcdir + "*_fq.sam"):
            base = fil.split('_')[-2]
            ...
            piRG = "java -Xmx1G -jar AddOrReplaceReadGroups.jar INPUT=" + fil + \
                   " OUTPUT=$UPDAT/Ch_" + base + "_updated.sam " + \
                   "RGID=\"" + str(counter) + "\" " + \
                   "RGLB=\"Aug12\" " + \
                   "RGPL=\"solid\" " + \
                   "RGPU=\"BC" + str(counter) + "\" " + \
            ...
            stVW = "samtools view -b -q 30 -S $UPDAT/Ch_" + base + \
                   "_updated.sam  " + \
                   "> $BAMS/" + base + ".bam && "
            out.write(stVW)
            ...
            stIN = "samtools index $SORT/" + base + "_sorted.bam && "
            out.write(stIN)

            stMP = "samtools mpileup -uEf $CRREF/Cre_comp_ref.fa " + \
                   "$SORT/" + base + "_sorted.bam > $BCFS/" + base + ".bcf && "
            out.write(stMP)
            ...
            pyFS = "python $VCFS/FilterVCFHaploid.py 30 " + \
                   "$VCFS/" + base + "_SAM.vcf " + \
                   "$VCFS/filtered_vcfs " + \
                   "## End of commands for sample " + base + "\n"
            out.write(pyFS)
